...
function handleMonthlyAmountInput(event) {
  ...
  monthlyAvailableAmount.value = limitedAmount;
  monthlyAmountError.value =
...
const selectedDeposit = computed(() =>
  deposits.value.find((deposit) => deposit.id === selectedDepositId.value),
);
...
function compareProfitLoss() {
  ...
  const request = {
    ...
    monthlyAvailableAmount: monthlyAvailableAmount.value,
  };

  console.log('손익 비교 요청:', request);

  // 추후 백엔드가 구현되면:
  // await api.post('/profit-loss/compare', request);
}
...
async function loadDeposits() {
  ...
  try {
    ...
    selectedDepositId.value = deposits.value[0]?.id ?? null;
  } catch (error) {
    ...
    <section class="calculator-card">
      ...
      <section class="form-section">
        ...
        <div v-else class="deposit-list">
          <button
            ...
            class="deposit-card"
            :class="{
              selected: selectedDepositId === deposit.id,
            }"
          ...
          >
            ...
            <span v-if="selectedDepositId === deposit.id" class="check-icon">
              ✓
            </span>
          </button>
        </div>
      ...
      <section class="loan-section">
        ...
        <div class="loan-options">
          <button
            type="button"
            :class="{ selected: selectedLoanType === 'lease' }"
            @click="selectedLoanType = 'lease'"
          >
            전세대출
          </button>
          ...
          <button
            type="button"
            :class="{ selected: selectedLoanType === 'credit' }"
            @click="selectedLoanType = 'credit'"
          >
            신용대출
          </button>
        </div>
