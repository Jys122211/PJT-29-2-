...
function answerQuestion(questionId, answer) {
  profitLossStore.setJeonseEligibilityAnswer(questionId, answer);
...
async function fetchQuestions() {
  ...
  try {
    const questions = await profitLossApi.getJeonseEligibilityQuestions();
    profitLossStore.setJeonseEligibilityQuestions(questions);
  } catch (error) {
    console.error('자격질문 로드 실패:', error);
    openResultModal('오류', '질문을 불러오는데 실패했습니다.');
  } finally {
...
async function continueToNextStep() {
  ...
  try {
    ...
    if (loanProductIds.length === 0) {
      openResultModal(
        '신청 가능한 상품 없음',
        '입력하신 신청 자격에 맞는 전세대출 상품이 없습니다.',
      );
      return;
    }

    router.push({ name: 'jeonsePreferential' });
  } catch (error) {
