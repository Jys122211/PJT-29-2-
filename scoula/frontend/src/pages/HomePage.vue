...
<template>
  <div class="dashboard">
    ...
    <svg viewBox="0 0 120 120" width="20" height="20">
      <line x1="25" y1="55" x2="95" y2="35" stroke="#ffbc00" stroke-width="9" stroke-linecap="round" />
      <path d="M60 28 L44 96 L76 96 Z" fill="#26221c" />
      <circle cx="60" cy="28" r="5" fill="#26221c" />
      <ellipse cx="60" cy="97" rx="20" ry="4" fill="#26221c" />
      <line x1="25" y1="55" x2="25" y2="84" stroke="#26221c" stroke-width="2.5" />
      <line x1="95" y1="35" x2="95" y2="64" stroke="#26221c" stroke-width="2.5" />
      <circle cx="25" cy="94" r="16" fill="#26221c" />
      <text x="25" y="95" fill="#fff" font-size="14" font-weight="700" text-anchor="middle" dominant-baseline="central">득</text>
      <circle cx="95" cy="74" r="13" fill="#d9d3c4" stroke="#26221c" stroke-width="1.5" />
    ...
    <div class="empty-deposit" v-else>
      ...
      <p class="empty-subtitle">예금과 대출을 등록하면<br />득실 비교를 시작할 수 있어요</p>
      <button class="empty-cta" @click="goToAssetRegister">자산 등록하기</button>
    </div>
    ...
    <Transition name="slide">
      <nav v-if="isMenuOpen" class="menu-drawer">
        <div class="menu-header">
          ...
          <button class="icon-btn menu-close" @click="isMenuOpen = false">☰</button>
        </div>

        <ul class="menu-list">
          <li class="menu-top">
            <router-link :to="{ name: 'home' }" @click="isMenuOpen = false">홈</router-link>
          </li>
        </ul>
        ...
        <div class="menu-group">
          ...
          <ul class="menu-sub">
            <li><router-link :to="{ name: 'comparisonInput' }" @click="isMenuOpen = false">새 득실 계산하기</router-link></li>
            <li><router-link :to="{ name: 'comparisonHistory' }" @click="isMenuOpen = false">나의 득실 기록</router-link></li>
          </ul>
        </div>
        ...
        <div class="menu-group">
          ...
          <ul class="menu-sub">
            <li><router-link :to="{ name: 'assetList' }" @click="isMenuOpen = false">보유 예금 목록</router-link></li>
            <li><router-link :to="{ name: 'assetRegister' }" @click="isMenuOpen = false">자산 등록</router-link></li>
          </ul>
        </div>
        ...
        <div class="menu-group">
          ...
          <ul class="menu-sub">
            <li><router-link :to="{ name: 'profile' }" @click="isMenuOpen = false">프로필 / 신용점수</router-link></li>
            <li><a href="#" @click.prevent="auth.logout(); isMenuOpen = false">로그아웃</a></li>
          </ul>
        </div>
      </nav>
    </Transition>
...
.empty-title { font-weight: 700; font-size: 16px; color: #26221c; }
.empty-subtitle { font-size: 13px; color: #a49a86; line-height: 1.5; }
.empty-cta {
  margin-top: 10px;
  padding: 10px 20px;
  background: #fff;
...
.asset-label { font-size: 12px; color: #a89f8e; }
.asset-amount { margin-top: 6px; font-weight: 700; font-size: 26px; letter-spacing: -0.5px; color: #fff; }
.asset-sub {
  margin-top: 12px;
  display: flex;
  gap: 14px;
...
.cta-text { flex: 1; margin-left: 14px; display: flex; flex-direction: column; gap: 4px; }
.cta-title { font-weight: 700; font-size: 15px; color: #4a4237; }
.cta-subtitle { font-size: 12px; color: #7a6a45; }
.cta-arrow { font-weight: 700; font-size: 18px; color: #7a6a45; }
...
.menu-title { flex: 1; font-weight: 700; font-size: 16px; color: #26221c; }
...
.menu-sub li a:hover { color: #26221c; }
